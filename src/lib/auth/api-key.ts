 ...
 * Format: sk-{48-char-hex} (24 random bytes → 48 hex chars after the prefix).
 */
...
/**
 * Extract a short prefix from the raw key (the first 8 hex chars after the known prefix).
...
export function extractKeyPrefix(rawKey: string): string {
  const keyPart = rawKey.startsWith('sk-')
    ? rawKey.slice(3)
    : rawKey.startsWith('imgw_')
      ? rawKey.slice(5)
      : rawKey
