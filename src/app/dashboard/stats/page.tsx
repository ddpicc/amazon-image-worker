...
export default function StatsPage() {
  ...
  if (loading) return <div className="flex items-center justify-center h-64"><div className="text-gray-500">Loading statistics...</div></div>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="rounded-lg shadow-sm border border-gray-200 bg-white p-4">
          ...
          {providers.length === 0 ? <div className="h-48 flex items-center justify-center text-gray-400 text-sm">No provider data available</div> : (
            <ResponsiveContainer width="100%" height={Math.max(200, providers.length * 40 + 40)}>
              <BarChart data={successData} layout="vertical" margin={{ left: 10, right: 20 }}>
                ...
                <Tooltip formatter={((value: number | string) => [`${value}%`, 'Success Rate']) as never} contentStyle={{ fontSize: 12 }} />
          ...
          {providers.length === 0 ? <div className="h-48 flex items-center justify-center text-gray-400 text-sm">No provider data available</div> : (
            <ResponsiveContainer width="100%" height={Math.max(200, providers.length * 40 + 40)}>
              <BarChart data={latencyData} layout="vertical" margin={{ left: 10, right: 20 }}>
                ...
                <YAxis type="category" dataKey="name" width={120} tick={{ fontSize: 12 }} />
                <Tooltip formatter={((value: number | string) => [`${value}ms`, 'Avg Latency']) as never} contentStyle={{ fontSize: 12 }} />
      ...
      <div className="rounded-lg shadow-sm border border-gray-200 bg-white">
        <div className="px-4 py-3 border-b border-gray-200"><h3 className="font-semibold text-gray-900">Provider Metrics Summary</h3></div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {providers.length === 0 ? <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400">No providers found</td></tr> : providers.map((p, i) => (
                <tr key={p.name} className={`${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-gray-50`}>
                  ...
                  <td className="px-4 py-2.5">{p.tripped ? <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">TRIPPED</span> : p.enabled ? <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">Active</span> : <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">Disabled</span>}</td>
                  ...
                  <td className="px-4 py-2.5 text-gray-600">{p.totalAttempts > 0 ? <span className={p.successRate >= 95 ? 'text-green-700' : p.successRate >= 80 ? 'text-yellow-700' : 'text-red-700'}>{p.successRate}%</span> : <span className="text-gray-400">N/A</span>}</td>
