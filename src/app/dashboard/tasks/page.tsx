...
export default function TasksPage() {
  ...
  async function handleRetry(_task: Task) {
    alert('Retry is not wired yet in the user/admin dashboard flow.')
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">{user?.role === 'ADMIN' ? 'Tasks' : 'My Tasks'}</h2>
          <p className="text-sm text-gray-500 mt-1">{user?.role === 'ADMIN' ? 'All tasks across the system (administrative view)' : 'Tasks from all of your API keys'}</p>
        </div>
      </div>
      ...
      {data && data.totalPages > 1 && (
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => setPage(Math.max(1, page - 1))} disabled={page <= 1} className="px-3 py-1.5 text-sm bg-gray-100 text-gray-600 rounded hover:bg-gray-200 disabled:opacity-50 transition-colors">Previous</button>
            <button onClick={() => setPage(Math.min(data.totalPages, page + 1))} disabled={page >= data.totalPages} className="px-3 py-1.5 text-sm bg-gray-100 text-gray-600 rounded hover:bg-gray-200 disabled:opacity-50 transition-colors">Next</button>
          </div>
        </div>
      )}
    </div>
